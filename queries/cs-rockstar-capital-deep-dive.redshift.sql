-- Rockstar Capital Management (cid=101469) — PO rejection deep-dive
-- Powers data/local/cs-rockstar-capital-deep-dive.json and knowledge/csm-quick-wins/rockstar-capital/
--
-- Run 2026-07-07. Sixth client-level deep-dive. Second chargeback-recovery client
-- after Incore Residential.
--
-- Why Rockstar matters:
--   - Was "insufficient signal" in v2 classifier (approvers write long free-form notes).
--   - v3 classifier caught 125 chargeback-recovery events — 70% dominance of classified.
--   - COMPLETELY DIFFERENT vocabulary than Incore:
--       Incore: "missing cbr", "any cbr?", "cbr $"
--       Rockstar: "upload fmo to show chargeback", "missing fmo to show previous resident charge back"
--   - Cross-vocabulary validation of the same underlying product gap (CBR field + doc attachment).
--   - 1,427 total rejections on 9,096 POs = 10.3% rejection rate.
--
-- HAVING thresholds: vendors >= 15 (smaller than Incore since lower PO volume), properties >= 25.
...
-- ============================================================================
-- 2) FMO decode — what's the full range of FMO / chargeback phrasing?
--    Rockstar-specific: characterize the vocabulary so we can validate v3 catches it all.
-- ============================================================================
WITH po_scope AS (
    SELECT h.id AS ap_header_id, h.cid
    FROM entrata_entrata.ap_headers h
...
-- ============================================================================
-- 3) Monthly rejection trend (13 months) — volume + rate over time
-- ============================================================================
WITH po_scope AS (
...
-- ============================================================================
-- 4) Top rejecters (users) — who's demanding the FMO docs?
-- ============================================================================
WITH po_scope AS (
    SELECT h.id AS ap_header_id, h.cid
    FROM entrata_entrata.ap_headers h
...
SELECT ahl.updated_by AS rejecter_user_id,
    ...
    SUM(CASE WHEN LOWER(ahl.approval_note) LIKE '%fmo%'
              OR LOWER(ahl.approval_note) LIKE '%chargeback%'
              OR LOWER(ahl.approval_note) LIKE '%charge back%'
              OR LOWER(ahl.approval_note) LIKE '%cbr%'
              THEN 1 ELSE 0 END) AS cbr_related_rejections
FROM entrata_entrata.ap_header_logs ahl
INNER JOIN po_scope ps ON ps.cid = ahl.cid AND ps.ap_header_id = ahl.ap_header_id
WHERE ahl.ap_header_type_id = 4
  AND ahl.log_datetime >= CURRENT_DATE - INTERVAL '13 months'
...
-- ============================================================================
-- 5) Top rejected vendors (min 15 rejections) — are CBR rejections vendor-clustered?
-- ============================================================================
WITH po_scope AS (
...
SELECT ps.ap_payee_id AS vendor_id,
    ...
    SUM(CASE WHEN COALESCE(pr.cbr_rej, 0) > 0 THEN 1 ELSE 0 END) AS cbr_rejected_pos
FROM po_scope ps
LEFT JOIN po_rej pr ON pr.cid = ps.cid AND pr.ap_header_id = ps.ap_header_id
GROUP BY ps.ap_payee_id
HAVING SUM(CASE WHEN COALESCE(pr.rej, 0) > 0 THEN 1 ELSE 0 END) >= 15
ORDER BY pos_rejected DESC
LIMIT 25;

-- ============================================================================
-- 6) Rejection by property (min 25 POs) — are CBR rejections property-clustered?
-- ============================================================================
WITH po_scope AS (
    SELECT h.id AS ap_header_id, h.cid
    FROM entrata_entrata.ap_headers h
...
-- SELECT id, first_name, last_name FROM entrata_entrata.company_users WHERE cid = 101469 AND id IN (<user_ids>);
-- SELECT id, company_name FROM entrata_entrata.ap_payees WHERE cid = 101469 AND id IN (<vendor_ids>);
-- SELECT id, property_name FROM entrata_entrata.properties WHERE cid = 101469 AND id IN (<property_ids>);
